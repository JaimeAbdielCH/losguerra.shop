...
class ModelLocalisationGeoZone extends Model {
	...
	public function getGeoZonesByZoneId($zone_id) {
		$geo_zone_data = $this->cache->get('geo_zone.' . (int)$zone_id);

		if (!$geo_zone_data) {
			$query = $this->db->query("SELECT *, (SELECT name FROM " . DB_PREFIX . "geo_zone WHERE geo_zone_id = a.geo_zone_id) name FROM " . DB_PREFIX . "zone_to_geo_zone a WHERE zone_id = '" . (int)$zone_id . "'");

			$geo_zone_data = $query->rows;

			$this->cache->set('zone.' . (int)$zone_id, $geo_zone_data);
		}
